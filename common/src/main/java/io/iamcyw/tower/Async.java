...
public class Async {
    ...
    public static <U> CompletableFuture<List<U>> each(List<CompletableFuture<U>> futures) {
        ...
        @SuppressWarnings("unchecked") CompletableFuture<U>[] arrayOfFutures = futures.toArray(
                new CompletableFuture[0]);
        CompletableFuture.allOf(arrayOfFutures).whenComplete((ignored, exception) -> {
            if (exception != null) {
    ...
    private static <T, U> void eachSequentiallyImpl(Iterator<T> iterator, CFFactory<T, U> cfFactory, int index,
                                                    List<U> tmpResult, CompletableFuture<List<U>> overallResult) {
        if (!iterator.hasNext()) {
