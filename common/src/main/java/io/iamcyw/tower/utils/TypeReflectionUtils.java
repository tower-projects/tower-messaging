...
public abstract class TypeReflectionUtils {
    ...
    private static Type[] getExactDirectSuperTypes(Type type) {
        ...
        logger.debug(type.getClass() + " is not supported for retrieving the exact direct super types from. Will by " +
    ...
    private static boolean isMissingTypeParameters(Type type) {
        if (type instanceof Class) {
            ...
            logger.debug(type.getClass() + " is not supported for checking if there are missing type parameters. " +
