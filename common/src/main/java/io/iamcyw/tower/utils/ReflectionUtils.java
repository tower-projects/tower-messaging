...
public abstract class ReflectionUtils {
    ...
    public static Class<?> resolvePrimitiveWrapperType(Class<?> primitiveType) {
        ...
        Assert.isTrue(primitiveType.isPrimitive(),
                      I18ns.create().content("primitiveType is not actually primitive: {}").args(primitiveType)
                           .apply());

        Class<?> primitiveWrapperType = primitiveWrapperTypeMap.get(primitiveType);
        Assert.nonNull(primitiveWrapperType,
                       I18ns.create().content("no wrapper found for primitiveType: {}").args(primitiveType).apply());
