...
public class SimpleQueryUpdateEmitter implements QueryUpdateEmitter {
    ...
    public boolean queryUpdateHandlerRegistered(SubscriptionQueryMessage<?, ?, ?> query) {
        return updateHandlers.keySet()
                .stream()
                .anyMatch(m -> m.getIdentifier()
                        .equals(query.getIdentifier()));
    }

    @Override
    public <U> UpdateHandlerRegistration<U> registerUpdateHandler(SubscriptionQueryMessage<?, ?, ?> query, int updateBufferSize) {
        Sinks.Many<SubscriptionQueryUpdateMessage<U>> sink = Sinks.many()
                .replay()
    ...
    @SuppressWarnings("unchecked")
    private <U> void doEmit(SubscriptionQueryMessage<?, ?, ?> query, SinkWrapper<?> updateHandler, SubscriptionQueryUpdateMessage<U> update) {
        ...
        try {
            ...
            logger.info(
                    "An error occurred while trying to emit an update to a query '{}'. " + "The subscription will be cancelled. Exception summary: {}",
    ...
    private boolean inStartedPhaseOfUnitOfWork() {
        return CurrentUnitOfWork.isStarted() && UnitOfWork.Phase.STARTED.equals(CurrentUnitOfWork.get()
                                                                                        .phase());
    }
    ...
    public static class Builder {
        ...
        public Builder updateMessageMonitor(MessageMonitor<? super SubscriptionQueryUpdateMessage<?>> updateMessageMonitor) {
            Assert.nonNull(updateMessageMonitor, () -> I18ns.create()
